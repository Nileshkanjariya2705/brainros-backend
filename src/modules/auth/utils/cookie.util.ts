...
export const LEGACY_ACCESS_COOKIE_NAME = 'accessToken';
...
export function getAccessCookieOptions(
  ...
  const sameSite: 'lax' | 'strict' | 'none' =
    sameSiteConfig === 'strict' || sameSiteConfig === 'none' || sameSiteConfig === 'lax'
      ? sameSiteConfig
      : isProd
        ? 'none'
        : 'lax';
...
/**
 * Sets both Access Token and Refresh Token as HTTP-only cookies on the response.
 */
...
/**
 * Clears both Access Token and Refresh Token HTTP cookies on the response (including legacy aliases).
 */
export function clearAuthCookies(
  ...
  res.clearCookie(LEGACY_ACCESS_COOKIE_NAME, clearOptions);
  res.clearCookie(LEGACY_REFRESH_COOKIE_NAME, clearOptions);
}
